...
while True:
    ...
    while (1):
        ...
        if sum(sum(seg)) == 0:
            break
        x, y, w, h = cv2.boundingRect(np.uint8(seg))
        if (w * h) > 0.1 * max_size:
            if sum(sum(seg)) > 100 * 255:
                obj[detected] = tracking_obj(np.uint8(seg), label)
                detected = detected + 1
            label = label + 1

    obj, max_label = relabel(obj, obj_prev, max_label)

    # Find Max Rectangle Size
    for i in range(0, detected):
        x, y, w, h = cv2.boundingRect(obj[i].mask)
        if (w * h) > max_size:
            max_size = (w * h)
